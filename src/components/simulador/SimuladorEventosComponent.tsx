...
const SimuladorEventosComponent = () => {
  ...
  const pontoEquilibrio = inputs.precoIngresso - inputs.custoVariavelPorPessoa > 0 
    ? Math.ceil(inputs.custoFixo / (inputs.precoIngresso - inputs.custoVariavelPorPessoa))
    : 0;
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div className="glass-card p-6 border-l-4 border-blue-500">
              ...
              <p className="text-xs text-gray-500 mt-2">
                {inputs.publico > 0 ? ((pontoEquilibrio / inputs.publico) * 100).toFixed(1) : 0}% do público estimado
              </p>
            </div>
          </div>
          ...
          <div className="glass-card p-6">
            ...
            <div className="w-full bg-gray-100 h-4 rounded-full overflow-hidden flex">
              <div 
                ...
                style={{ width: `${receitaTotal > 0 ? (inputs.custoFixo / receitaTotal) * 100 : 0}%` }}
                title="Custos Fixos"
              />
              <div 
                className="bg-orange-400 h-full" 
                style={{ width: `${receitaTotal > 0 ? (custoVariavelTotal / receitaTotal) * 100 : 0}%` }}
                title="Custos Variáveis"
              />
              <div 
                className="bg-primary h-full" 
                style={{ width: `${receitaTotal > 0 ? Math.max(0, (lucroEstimado / receitaTotal) * 100) : 0}%` }}
                title="Lucro"
              />
            </div>
